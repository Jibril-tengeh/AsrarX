...
// Global variable to track whether the first initial load has been shown/completed
let appOpened = false;
...
export const AsrarHubLoader: React.FC<AsrarHubLoaderProps> = ({ 
  ...
  const [isInitial, setIsInitial] = useState(!appOpened);

  useEffect(() => {
    // If it was the first time rendering the loader, mark appOpened as true so subsequent renders don't show the full logo
    appOpened = true;
  }, []);
  ...
  const sizeClasses = {
    ...
    md: 'w-36 h-36',
    lg: 'w-56 h-56',
    fullscreen: 'fixed inset-0 z-50 flex items-center justify-center bg-black'
  };

  const [imgFailed, setImgFailed] = React.useState(false);

  // If this is NOT the initial startup load, return a simple, elegant lightweight spinner instead of loading the branding image
  if (!isInitial) {
    if (size === 'fullscreen') {
      return (
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-white/70 dark:bg-gray-900/70 backdrop-blur-md">
          <div className="flex flex-col items-center gap-3">
            ...
            <span className="text-xs font-semibold text-emerald-600 dark:text-emerald-400">Chargement...</span>
          </div>
        </div>
      );
    }
  ...
  const loaderContent = (
    ...
    >
      {!imgFailed ? (
        ...
        <svg 
          viewBox="0 0 240 140" 
          className="w-64 sm:w-80 h-auto filter drop-shadow-[0_0_20px_rgba(229,169,60,0.15)]" 
          fill="none" 
        ...
        >
          ...
          <defs>
            ...
            <linearGradient id="goldGradientBright" x1="0%" y1="0%" x2="100%" y2="100%">
              ...
              <stop offset="35%" stopColor="#FFE09E" />
              <stop offset="70%" stopColor="#F5C368" />
              <stop offset="100%" stopColor="#B3862B" />
            </linearGradient>
