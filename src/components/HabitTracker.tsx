      ...
      {showReminders ? (
        <div className="space-y-4">
          ...
          {reminders.map((r, rIdx) => (
            <div key={r.id ? `reminder-${r.id}-${rIdx}` : `reminder-${rIdx}`} className="flex justify-between items-center bg-gray-50 dark:bg-gray-900/50 p-3 rounded-xl border border-gray-100 dark:border-gray-700/50">
              ...
              <div className="flex gap-1" style={{ width: 'max-content' }}>
                ...
                <div className="flex gap-1">
                  {Array.from({ length: 12 }).map((_, weekIndex) => (
                    <div key={`habit-week-${weekIndex}`} className="flex flex-col gap-1">
                      {Array.from({ length: 7 }).map((_, dayIndex) => {
                        const dataIndex = weekIndex * 7 + dayIndex;
                        const item = activityData[dataIndex];
                        if (!item) return <div key={`habit-empty-${weekIndex}-${dayIndex}`} className="w-4 h-4 rounded-sm" />;
                        return (
                          <div 
                            key={`habit-cell-${weekIndex}-${dayIndex}`}
