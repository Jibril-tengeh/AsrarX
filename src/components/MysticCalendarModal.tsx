...
import { X, ChevronLeft, ChevronRight, Sparkles, BookOpen, Compass, Moon, Info, Eye, EyeOff, Calendar, Download, ChevronDown, Activity, Heart, Zap, Brain, Timer, Play, Pause, Flame, Check, ShieldAlert, RefreshCw, Lock, Copy, ExternalLink, Leaf, Sliders } from 'lucide-react';
        ...
        >
          ...
          <div className={`flex items-center justify-between mb-4 pb-3 border-b transition-colors duration-300 ${
            ...
            <div className="flex items-center gap-2">
              {isAdmin && (
                <button
                  onClick={() => setShowAdminScaleControls(!showAdminScaleControls)}
                  className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-[10px] font-bold tracking-wider transition-all cursor-pointer ${
                    showAdminScaleControls
                      ? 'bg-amber-500 text-white shadow-md'
                      : 'bg-amber-500/10 text-amber-600 dark:text-amber-400 hover:bg-amber-500/20 border border-amber-500/20'
                  }`}
                  title="Ajuster la taille du calendrier et des cartes"
                >
                  <Sliders size={12} />
                  <span>Admin Taille</span>
                </button>
              )}
          ...
          {/* Admin Scaling Control Panel */}
          {isAdmin && showAdminScaleControls && (
            <div className="mb-4 p-3 bg-amber-500/10 dark:bg-amber-950/30 border border-amber-500/30 rounded-2xl space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-amber-700 dark:text-amber-300 flex items-center gap-1.5">
                  <Sliders size={14} /> Réglage des Tailles du Calendrier (Admin)
                </span>
                <button
                  type="button"
                  onClick={resetScales}
                  className="px-2 py-0.5 bg-amber-500/20 hover:bg-amber-500/30 text-amber-800 dark:text-amber-200 rounded-lg text-[10px] font-bold transition-colors cursor-pointer"
                >
                  Réinitialiser (100%)
                </button>
              </div>
                ...
                <div className="p-2.5 bg-white/80 dark:bg-gray-800/80 rounded-xl border border-amber-500/20 space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="px-2 py-0.5 bg-gray-200 dark:bg-gray-700 hover:bg-amber-500 hover:text-white rounded text-[10px] font-bold cursor-pointer"
                    >
                      -5%
                    </button>
                    <input
          ...
          {(isCalendarBlocked || isCalendarMaintenance || isCalendarPremiumLocked) ? (
            ...
            <div
              style={{
                transform: subCardScale !== 1 ? `scale(${subCardScale})` : undefined,
                transformOrigin: 'top center',
              }}
              className="w-full flex flex-col transition-transform duration-200"
            >
