...
export const Store: React.FC = () => {
          ...
          async (reference) => {
            try {
              ...
              await updateDoc(doc(db, 'users', user.uid), {
                purchasedItems: arrayUnion(product.id)
              });
            } catch (err) {
              console.error("Failed to add purchased product to user doc:", err);
            }
  ...
  return (
    <div 
      style={{
        paddingTop: `max(0px, calc(16px + var(--feed-store-offset, 0px) + var(--feed-global-offset, 0px)))`
      }}
      className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pb-24 safe-area-pt transition-all duration-300"
    >
