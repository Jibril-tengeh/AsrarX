...
export default function DiscretionMentalProtection() {
  ...
  const t = DISCRETION_TRANSLATIONS[langKey] || DISCRETION_TRANSLATIONS.fr;

  const [activeTab, setActiveTab] = useState<'khumm' | 'hissn' | 'tarkib' | 'usturlab'>('khumm');

  const handleTabChange = (tabId: 'khumm' | 'hissn' | 'tarkib' | 'usturlab') => {
    setActiveTab(tabId);
    setTimeout(() => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-slate-950 via-slate-900 to-indigo-950 p-6 sm:p-8 text-white shadow-2xl border border-slate-800">
        ...
        <ShieldCheck size={280} className="absolute -right-12 -bottom-20 text-slate-800/20 pointer-events-none" />
      </div>

      {/* Navigation Tabs */}
      <div className="flex flex-wrap gap-2 border-b border-gray-200 dark:border-slate-800 pb-3">
        {[
          { id: 'khumm', label: t.tabs.khatamKhumm, icon: EyeOff },
          { id: 'hissn', label: t.tabs.hissnAql, icon: Brain },
          { id: 'tarkib', label: t.tabs.tarkibHarfi, icon: Feather },
          { id: 'usturlab', label: t.tabs.usturlabAsrar, icon: Compass }
        ].map((tab) => {
          const Icon = tab.icon;
      ...
      <div ref={detailsRef} className="transition-all duration-300 scroll-mt-6">
        {activeTab === 'khumm' && <KhatamKhummTab t={t} />}
        {activeTab === 'hissn' && <HissnAqlTab t={t} />}
        {activeTab === 'tarkib' && <TarkibHarfiTab t={t} />}
        {activeTab === 'usturlab' && <UsturlabAsrarTab t={t} />}
      </div>
