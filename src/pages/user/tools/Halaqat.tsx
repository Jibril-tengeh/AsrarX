...
import { Users, Target, Activity, ChevronLeft, Hand, Award, Plus, MapPin, Globe, Check, AlertCircle, Info, Flame, Sparkles, X } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { db } from '../../../lib/firebase';
import { useLanguage } from '../../../contexts/LanguageContext';
import { doc, onSnapshot, updateDoc, increment, setDoc, collection, query, where, getDocs, addDoc } from 'firebase/firestore';
                ...
                {circles.map((c) => {
                  ...
                  return (
                    <motion.div
                      whileHover={{ y: -4 }}
                      key={c.id}
                      className="bg-white dark:bg-gray-800 rounded-3xl p-6 border border-gray-100 dark:border-gray-750 shadow-sm hover:shadow-md transition-all flex flex-col justify-between"
                    >
            ...
            <div className="lg:col-span-2 bg-white dark:bg-gray-850 rounded-3xl p-6 sm:p-8 border border-gray-100 dark:border-gray-750 shadow-sm flex flex-col items-center justify-center space-y-8">
              ...
              <div className="relative my-4 flex items-center justify-center">
                ...
                <motion.button
                  ...
                  className={`w-48 h-48 sm:w-56 sm:h-56 rounded-full flex flex-col items-center justify-center text-white relative group shadow-2xl transition-all cursor-pointer ${
                    activeCircle.count >= activeCircle.target
                      ? 'bg-gradient-to-br from-amber-500 to-orange-600 shadow-amber-500/20'
                      : 'bg-gradient-to-br from-emerald-500 via-emerald-600 to-teal-700 shadow-emerald-500/20'
                  }`}
                >
                  <div className="absolute inset-0 rounded-full border-4 border-white/20 scale-105 group-hover:scale-110 transition-transform"></div>
                  <Hand size={44} className="opacity-80 mb-3 animate-pulse" />
        ...
        {showJoinPromptModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              animate={{ scale: 1, y: 0, opacity: 1 }}
              exit={{ scale: 0.95, y: 15, opacity: 0 }}
              className="relative w-full max-w-md bg-white dark:bg-gray-900 rounded-3xl p-6 shadow-2xl"
            >
              <div className="flex justify-between items-center mb-6">
