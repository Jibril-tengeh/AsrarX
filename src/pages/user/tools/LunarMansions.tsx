...
import { Moon, ArrowLeft, Star, Compass, Clock, Info, Search, Filter, Calendar, Sparkles, Copy, Check, Shield, Heart, Briefcase, Plane, Activity, Feather, RefreshCw, BookOpen } from 'lucide-react';
...
const UI_LABELS: Record<string, any> = {
  fr: {
    ...
    degree: "Coordonnées / Degrés Zodiaces",
    ...
    scrollSub: "Exporter les symboles, invocations et serviteurs sous forme de parchemin ancien",
    copied: "Copié dans le presse-papier !",
    copyScroll: "Copier le texte du Parchemin",
    love: "Amour & Mariage",
  ...
  en: {
    ...
    copyScroll: "Copy Parchment Text",
    love: "Love & Marriage",
  ...
  ha: {
    ...
    copied: "An kofa a clipboard!",
    copyScroll: "Kwandaf da rubutu",
...
export const LunarMansions: React.FC = () => {
  ...
  return (
    <div className="max-w-5xl mx-auto p-4 sm:p-6 lg:p-8 safe-area-pt pb-24">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 dark:text-white flex items-center gap-3">
              <Moon className="text-indigo-500 shrink-0" size={32} />
              {labels.title}
            </h1>
            <p className="text-gray-500 dark:text-gray-400 mt-1 text-sm sm:text-base">{labels.subtitle}</p>
          </div>

          <span className="self-start sm:self-auto bg-indigo-50 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800 text-xs font-semibold px-3 py-1.5 rounded-full flex items-center gap-1.5">
            <Sparkles size={14} />
            {labels.system}
          </span>
        </div>
      ...
      {/* Tabs */}
      <div className="flex overflow-x-auto gap-2 pb-2 mb-6 border-b border-gray-200 dark:border-gray-800 scrollbar-none">
        {[
      ...
      {activeTab === 'overview' && (
        <>
          ...
          {activeMansion && (
            <div className="mb-8">
              <motion.div
                ...
                className="bg-gradient-to-br from-indigo-900 via-slate-900 to-indigo-950 rounded-2xl sm:rounded-3xl p-5 sm:p-8 shadow-2xl text-white relative overflow-hidden border border-indigo-700/40"
              >
                ...
                <div className="relative z-10">
                  <div className="flex items-center justify-between gap-2 text-indigo-300 font-medium mb-3 text-xs sm:text-sm">
                    <span className="flex items-center gap-1.5">
                      <Clock size={16} />
                      {activeMansion.id === activeTodayMansion?.id ? labels.activeToday : `${labels.mansion} #${activeMansion.id}`}
                    </span>
                    <span className="text-indigo-400 text-xs bg-indigo-950/60 px-2.5 py-1 rounded-md border border-indigo-800">
                  ...
                  <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-6">
                    <div>
                      <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-1 break-words tracking-tight">{activeMansion.name}</h2>
                      <p className="text-2xl sm:text-3xl font-arabic text-amber-300 mt-1" dir="rtl">{activeMansion.arabic}</p>
                    </div>
                    <div className="flex flex-wrap gap-2">
                      ...
                      <span className="bg-white/10 px-3 py-1.5 rounded-full text-xs sm:text-sm font-bold text-indigo-200 border border-white/10">
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
                        <div className="bg-indigo-950/60 rounded-xl p-3.5 border border-indigo-800/60">
                          <span className="text-[11px] text-indigo-300 font-semibold block mb-0.5">{labels.angel}</span>
                          <span className="text-sm font-bold text-white block">{angelName}</span>
                          <span className="text-xs font-arabic text-amber-300 block">{extra.angelAr}</span>
                        </div>
                        <div className="bg-indigo-950/60 rounded-xl p-3.5 border border-indigo-800/60">
                          <span className="text-[11px] text-indigo-300 font-semibold block mb-0.5">{labels.asma}</span>
                          <span className="text-sm font-bold text-emerald-300 block">{asmaName}</span>
                          <span className="text-xs font-arabic text-amber-300 block">{extra.asmaAr}</span>
                        </div>
                        <div className="bg-indigo-950/60 rounded-xl p-3.5 border border-indigo-800/60">
                          <span className="text-[11px] text-indigo-300 font-semibold block mb-0.5">{labels.incense}</span>
                          <span className="text-xs font-bold text-amber-200 block">{incenseName}</span>
                        </div>
                        <div className="bg-indigo-950/60 rounded-xl p-3.5 border border-indigo-800/60">
                          <span className="text-[11px] text-indigo-300 font-semibold block mb-0.5">{labels.sadaqah}</span>
                          <span className="text-xs font-bold text-indigo-200 block">{sadaqahName}</span>
                        </div>
                      </div>
                    );
                  })()}
                  ...
                  {/* Mansion Description */}
                  <div className="bg-indigo-950/40 rounded-xl sm:rounded-2xl p-4 sm:p-5 mb-6 border border-indigo-800/50 backdrop-blur-sm">
                    <h3 className="text-indigo-200 font-bold mb-2 flex items-center gap-2 text-sm sm:text-base">
                      <Info size={18} className="text-indigo-400" /> {labels.desc}
                    </h3>
                    <p className="text-indigo-50 leading-relaxed text-sm sm:text-base break-words">
                      {activeMansion.desc}
                    </p>
                  </div>
                  ...
                  {(() => {
                    ...
                    const wirdName = currentLang === 'ha' ? extra.wirdNameHa : currentLang === 'en' ? extra.wirdNameEn : extra.wirdNameFr;
                    return (
                      <div className="bg-gradient-to-r from-amber-950/40 via-indigo-950/60 to-emerald-950/40 rounded-xl sm:rounded-2xl p-4 sm:p-5 mb-6 border border-amber-500/30">
                        <div className="flex justify-between items-center mb-2">
                          <h3 className="text-amber-300 font-bold text-sm sm:text-base flex items-center gap-2">
                            <BookOpen size={18} className="text-amber-400" /> {labels.wird}
                          </h3>
                          <span className="bg-amber-500/20 text-amber-300 font-mono text-xs font-bold px-2.5 py-1 rounded-lg border border-amber-500/30">
                            {extra.wirdCount}x
                          </span>
                        </div>
                        <p className="text-xl sm:text-2xl font-arabic text-amber-200 text-center my-3 leading-loose" dir="rtl">
                          {extra.wirdAr}
                        </p>
                        <p className="text-xs sm:text-sm text-indigo-200 text-center italic">
                          {wirdName}
                        </p>
                      </div>
                    );
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                    <div className="bg-indigo-950/50 rounded-xl sm:rounded-2xl p-4 sm:p-5 border border-indigo-800/80">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl sm:rounded-2xl p-4 mb-6 shadow-sm border border-gray-200 dark:border-gray-700">
            <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4">
              ...
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-4 py-2 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:text-white"
                />
              </div>
              ...
              <div className="flex flex-wrap items-center gap-3">
                <div className="flex items-center gap-1 text-xs">
                  <span className="text-gray-500 dark:text-gray-400 font-semibold">{labels.filterNature}</span>
                  <select
                    value={natureFilter}
                    onChange={(e) => setNatureFilter(e.target.value)}
                    className="bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg px-2.5 py-1.5 text-xs font-bold text-gray-700 dark:text-gray-300 focus:outline-none"
                  >
                ...
                <div className="flex items-center gap-1 text-xs">
                  <span className="text-gray-500 dark:text-gray-400 font-semibold">{labels.filterElem}</span>
                  <select
                    value={elementFilter}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl sm:rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
            <div className="p-4 sm:p-5 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center">
              <h2 className="text-lg font-bold text-gray-900 dark:text-white">{labels.select}</h2>
      ...
      {activeTab === 'advisor' && (
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-200 dark:border-gray-700">
          <div className="mb-6">
            ...
            <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">{labels.advisorSub}</p>
          </div>

          <div className="mb-6">
            <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-3">{labels.selectIntent}</label>
          ...
          {(() => {
            ...
            return (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Recommended */}
                <div className="bg-emerald-50/60 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-800 rounded-2xl p-5">
                  <h3 className="text-emerald-800 dark:text-emerald-300 font-bold text-base mb-4 flex items-center gap-2">
                    ...
                    {advisor.rec.map(mId => {
                      ...
                      return (
                        <div 
                          ...
                          className="bg-white dark:bg-gray-800 p-3.5 rounded-xl border border-emerald-100 dark:border-emerald-800/40 shadow-sm hover:border-emerald-400 transition-all cursor-pointer"
                        >
                          <div className="flex justify-between items-center mb-1">
                            <span className="text-xs font-bold text-emerald-700 dark:text-emerald-400">#{locM.id} {locM.name}</span>
                            <span className="text-sm font-arabic text-amber-500" dir="rtl">{locM.arabic}</span>
                          </div>
                          <p className="text-xs text-gray-600 dark:text-gray-300 line-clamp-2">{locM.desc}</p>
                          <div className="mt-2 text-[11px] text-indigo-600 dark:text-indigo-400 font-semibold flex items-center gap-1">
                            <Sparkles size={12} /> {extra.asmaFr}
                          </div>
                        </div>
                      );
                    })}
                ...
                {/* Avoid */}
                <div className="bg-rose-50/60 dark:bg-rose-950/20 border border-rose-200 dark:border-rose-800 rounded-2xl p-5">
                  <h3 className="text-rose-800 dark:text-rose-300 font-bold text-base mb-4 flex items-center gap-2">
                    <Shield size={20} className="text-rose-600 dark:text-rose-400" />
                    {labels.avoidMansions}
                  </h3>
                  <div className="space-y-3">
                    {advisor.avoid.map(mId => {
                      ...
                      return (
                        <div 
                          ...
                          className="bg-white dark:bg-gray-800 p-3.5 rounded-xl border border-rose-100 dark:border-rose-800/40 shadow-sm hover:border-rose-400 transition-all cursor-pointer"
                        >
                          <div className="flex justify-between items-center mb-1">
                            <span className="text-xs font-bold text-rose-700 dark:text-rose-400">#{locM.id} {locM.name}</span>
                            <span className="text-sm font-arabic text-gray-500" dir="rtl">{locM.arabic}</span>
                          </div>
                          <p className="text-xs text-gray-600 dark:text-gray-300 line-clamp-2">{locM.desc}</p>
                        </div>
                      );
      ...
      {activeTab === 'calculator' && (
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-200 dark:border-gray-700">
          <div className="mb-6">
            ...
            <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">{labels.calcSub}</p>
          </div>

          <div className="max-w-md bg-gray-50 dark:bg-gray-900 p-4 rounded-xl border border-gray-200 dark:border-gray-700 mb-6">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-3 py-2 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg text-sm text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500"
              />
              <button
                onClick={handleCalculateDate}
                className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs rounded-lg transition-colors flex items-center gap-1.5 cursor-pointer"
              >
                <RefreshCw size={14} />
                {labels.calcBtn}
              </button>
          ...
          {calcResultMansion && (
            <motion.div
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              className="bg-indigo-950 text-white rounded-2xl p-6 border border-indigo-800"
            >
              <div className="flex justify-between items-start mb-4">
                <div>
                  ...
                  <h3 className="text-2xl font-bold text-white">{calcResultMansion.name}</h3>
                  <p className="text-xl font-arabic text-amber-300 mt-1" dir="rtl">{calcResultMansion.arabic}</p>
                </div>
                <button
                  onClick={() => {
              ...
              <div className="bg-indigo-900/50 p-4 rounded-xl border border-indigo-800 mb-4 text-xs sm:text-sm text-indigo-100">
              ...
              {(() => {
                ...
                return (
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs">
                    <div className="bg-indigo-900/30 p-2.5 rounded-lg border border-indigo-800/40">
                      <span className="text-indigo-400 block font-semibold">{labels.degree}</span>
                      <span className="font-bold text-white block mt-0.5">{extra.degree}</span>
                    </div>
                    <div className="bg-indigo-900/30 p-2.5 rounded-lg border border-indigo-800/40">
                      <span className="text-indigo-400 block font-semibold">{labels.angel}</span>
                      <span className="font-bold text-amber-300 block mt-0.5">{extra.angelFr} ({extra.angelAr})</span>
                    </div>
                    <div className="bg-indigo-900/30 p-2.5 rounded-lg border border-indigo-800/40 col-span-2 sm:col-span-1">
                      <span className="text-indigo-400 block font-semibold">{labels.asma}</span>
                      <span className="font-bold text-emerald-300 block mt-0.5">{extra.asmaFr}</span>
                    </div>
                  </div>
                );
              })()}
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-200 dark:border-gray-700">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            <div>
              ...
              <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">{labels.scrollSub}</p>
            </div>

            <button
              onClick={handleCopyScrollText}
              className="px-4 py-2.5 bg-amber-600 hover:bg-amber-700 text-white font-bold text-xs sm:text-sm rounded-xl shadow-md transition-all flex items-center gap-2 cursor-pointer self-start sm:self-auto"
            >
          ...
          {(() => {
            ...
            return (
              <div className="relative bg-[#fbf6e9] dark:bg-[#1a1612] text-[#3d2f21] dark:text-[#e8d7c3] rounded-3xl p-6 sm:p-10 border-8 border-[#d4af37]/40 shadow-2xl overflow-hidden font-serif">
                {/* Vintage Corner Ornaments */}
                <div className="absolute top-2 left-2 text-[#d4af37] text-2xl font-bold select-none">❖</div>
                <div className="absolute top-2 right-2 text-[#d4af37] text-2xl font-bold select-none">❖</div>
                <div className="absolute bottom-2 left-2 text-[#d4af37] text-2xl font-bold select-none">❖</div>
                <div className="absolute bottom-2 right-2 text-[#d4af37] text-2xl font-bold select-none">❖</div>

                <div className="text-center border-b-2 border-[#d4af37]/40 pb-6 mb-6">
                  <span className="text-xs font-bold tracking-widest uppercase text-[#8b5e34] dark:text-[#d4af37] block mb-1">
                    Manzil al-Qamar #{activeMansion.id}
                  </span>
                  <h3 className="text-3xl sm:text-4xl font-extrabold text-[#2a1f17] dark:text-[#f3e5d8] tracking-tight">
                    {activeMansion.name}
                  </h3>
                  <p className="text-3xl sm:text-4xl font-arabic text-[#b8860b] dark:text-[#e6c662] mt-2" dir="rtl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                  <div className="space-y-3 text-sm">
                    <div className="flex items-center justify-between border-b border-[#d4af37]/20 pb-1.5">
                      <span className="font-bold text-[#6b4724] dark:text-[#c4a482]">{labels.angel}:</span>
                      <span className="font-bold text-amber-700 dark:text-amber-300">{angelName} ({extra.angelAr})</span>
                    </div>
                    <div className="flex items-center justify-between border-b border-[#d4af37]/20 pb-1.5">
                      <span className="font-bold text-[#6b4724] dark:text-[#c4a482]">{labels.asma}:</span>
                      <span className="font-bold text-emerald-700 dark:text-emerald-400">{asmaName} ({extra.asmaAr})</span>
                    </div>
                    <div className="flex items-center justify-between border-b border-[#d4af37]/20 pb-1.5">
                      <span className="font-bold text-[#6b4724] dark:text-[#c4a482]">{labels.element}:</span>
                  ...
                  <div className="space-y-3 text-sm">
                    <div className="flex items-center justify-between border-b border-[#d4af37]/20 pb-1.5">
                      <span className="font-bold text-[#6b4724] dark:text-[#c4a482]">{labels.incense}:</span>
                      <span className="font-bold text-[#8b5e34] dark:text-[#d4af37]">{incenseName}</span>
                    </div>
                    <div className="flex items-center justify-between border-b border-[#d4af37]/20 pb-1.5">
                      <span className="font-bold text-[#6b4724] dark:text-[#c4a482]">{labels.sadaqah}:</span>
                      <span className="font-bold">{sadaqahName}</span>
                    </div>
                    <div className="flex items-center justify-between border-b border-[#d4af37]/20 pb-1.5">
                      <span className="font-bold text-[#6b4724] dark:text-[#c4a482]">{labels.degree}:</span>
                      <span className="font-mono text-xs">{extra.degree}</span>
                    </div>
                  </div>
                </div>

                {/* Central Sacred Calligraphy & Invocation */}
                <div className="bg-[#f2e7d3] dark:bg-[#26201a] p-5 rounded-2xl border border-[#d4af37]/40 text-center my-6 shadow-inner">
                  <span className="text-xs font-bold uppercase tracking-widest text-[#8b5e34] dark:text-[#d4af37] block mb-2">
                    {labels.wird} ({extra.wirdCount}x)
                  </span>
                  <p className="text-2xl sm:text-3xl font-arabic text-[#2a1f17] dark:text-[#f3e5d8] leading-loose my-2" dir="rtl">
                    {extra.wirdAr}
                  </p>
                  <p className="text-xs italic text-[#6b4724] dark:text-[#c4a482]">
                    {wirdName}
                  </p>
                </div>

                <p className="text-xs sm:text-sm leading-relaxed italic text-center text-[#523d2b] dark:text-[#c2b19c]">
                  "{activeMansion.desc}"
                </p>
              </div>
            );
