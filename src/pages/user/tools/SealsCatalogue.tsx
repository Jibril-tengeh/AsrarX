...
export const SealsCatalogue: React.FC = () => {
  ...
  const getStatusBadge = (status: SealStatus) => {
    switch (status) {
      ...
      case 'disabled':
        return (
          <span className="bg-gray-800 text-gray-400 border border-gray-700 text-[10px] font-bold px-2 py-0.5 rounded-full">
            Désactivé
          </span>
        );
      default:
  ...
  return (
    <div className="min-h-screen bg-[#070210] text-gray-100 pb-16 pt-6 px-3 sm:px-6">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex items-center justify-between gap-4">
          <Link
            to="/tools"
            className="inline-flex items-center gap-2 text-xs font-bold text-amber-400 hover:text-amber-300 transition-colors bg-purple-950/50 hover:bg-purple-900/60 px-3.5 py-2 rounded-xl border border-purple-500/30"
          >
            <ArrowLeft size={16} />
          ...
          {isAdmin && (
            <button
              ...
              className="inline-flex items-center gap-2 text-xs font-bold text-amber-300 bg-amber-500/10 hover:bg-amber-500/20 px-3.5 py-2 rounded-xl border border-amber-500/40 transition-all cursor-pointer"
            >
              <Sliders size={16} />
        ...
        <AnimatePresence>
          {showAdminControl && (
            <motion.div
              ...
              className="overflow-hidden bg-purple-950/60 border border-amber-500/50 rounded-3xl p-5 space-y-4 shadow-2xl"
            >
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-amber-500/30">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className={`px-3.5 py-1.5 text-xs font-bold rounded-xl border transition-all cursor-pointer ${
                      globalMaintenance
                        ...
                        : 'bg-black/80 text-gray-300 border-gray-600 hover:text-white'
                    }`}
                  >
                    {globalMaintenance
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 text-xs font-bold rounded-xl bg-gray-800 hover:bg-gray-700 text-gray-300 border border-gray-600 cursor-pointer flex items-center gap-1.5"
                  >
                    <RefreshCw size={14} />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5 max-h-80 overflow-y-auto pr-1">
                {LUNAR_SEAL_VARIETIES.map((seal) => {
                  ...
                  return (
                    <div
                      key={seal.id}
                      className="flex items-center justify-between gap-2 bg-black/80 border border-purple-500/30 p-3 rounded-2xl"
                    >
                      ...
                      <div className="flex items-center gap-1.5 flex-shrink-0">
                        <select
                          ...
                          className="bg-purple-950 text-amber-200 text-xs font-bold py-1.5 px-2 rounded-xl border border-purple-500/40 cursor-pointer outline-none"
                        >
                        ...
                        <select
                          ...
                          className="bg-black text-gray-300 text-xs font-bold py-1.5 px-2 rounded-xl border border-gray-700 cursor-pointer outline-none"
                        >
        ...
        {/* Filter Controls Bar */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-purple-950/30 border border-purple-500/30 p-4 rounded-3xl">
          {/* Search Box */}
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-black/70 border border-purple-500/30 rounded-2xl pl-10 pr-4 py-2.5 text-xs sm:text-sm text-gray-100 placeholder-purple-400 focus:outline-none focus:border-amber-400 transition-all"
            />
          </div>
            ...
            <button
              ...
              className={`py-2 px-3.5 text-xs font-extrabold rounded-xl transition-all whitespace-nowrap cursor-pointer ${
                selectedGroup === 'all'
                  ...
                  : 'bg-black/60 text-purple-200 border border-purple-500/30 hover:bg-purple-900/40'
              }`}
            >
              Tous ({seals.length})
            </button>
          ...
          <div className="lg:col-span-5 space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-2.5 max-h-[750px] overflow-y-auto pr-1">
              {filteredSeals.map((seal) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-serif text-sm font-bold flex-shrink-0 border ${
                        isSelected 
                          ...
                          : 'bg-purple-950/80 text-amber-300 border-purple-500/30'
                      }`}>
                        {seal.groupId}
                      </div>
                    ...
                    <span className="text-sm font-serif font-semibold text-amber-300 flex-shrink-0" dir="rtl">
                      {seal.arabicName}
                    </span>
                  </button>
                );
              ...
              {filteredSeals.length === 0 && (
                <div className="text-center py-12 bg-black/40 border border-purple-500/20 rounded-2xl p-6 text-purple-300 text-xs">
            ...
            {activeSeal && (
              <div className="bg-gradient-to-b from-purple-950/50 via-[#100422] to-black border border-purple-500/40 rounded-3xl p-5 sm:p-7 shadow-2xl space-y-6">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-5 border-b border-purple-500/30">
                  <div className="space-y-1">
                    ...
                    {(activeSeal as any).scienceOrigin && (
                      <span className="inline-block text-[11px] font-mono font-bold text-amber-400 bg-black/60 px-2.5 py-0.5 rounded-lg border border-amber-500/30">
                        {(activeSeal as any).scienceOrigin}
                      </span>
                    )}
                  </div>
                ...
                {activeSeal.status === 'premium' && !isPremium && !isAdmin && (
                  <div className="bg-amber-950/50 border border-amber-500/50 rounded-2xl p-5 text-center text-amber-200 space-y-3 shadow-lg">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2">
                      ...
                      <button
                        ...
                        className="flex items-center justify-center gap-2 bg-black/80 hover:bg-gray-900 text-gray-200 border border-gray-700 font-extrabold py-3 px-4 rounded-2xl shadow-lg transition-all cursor-pointer disabled:opacity-50 text-xs"
                      >
                        {copiedSymbol ? (
                          <>
          ...
          >
            ...
            >
              <button
                type="button"
                onClick={() => setIsFullScreen(false)}
                className="absolute top-4 right-4 text-gray-400 hover:text-white bg-black/60 p-2 rounded-full border border-gray-700"
              >
                ✕
              </button>
              ...
              <div className="text-center space-y-2">
                ...
                <p className="text-lg font-serif text-amber-300" dir="rtl">
                  {activeSeal.arabicName}
                </p>
              </div>
