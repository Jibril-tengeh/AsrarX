...
export const Tasbih: React.FC = () => {
  ...
  useEffect(() => {
    // Load state from local storage and params
    try {
      let customArr: Zikr[] = [];
      ...
      if (savedSettings) {
        ...
        if (parsedSettings && typeof parsedSettings === 'object') {
          const { sound, vibe, soundStyle: savedStyle, soundTriggerMode: savedTriggerMode, lastActiveId: savedLastActiveId } = parsedSettings;
          ...
          lastActiveId = savedLastActiveId || null;
        }
      }

      // Priority 1: Check URL search parameters (?name=...&arabic=...&target=...)
      const params = new URLSearchParams(location.search);
      const nameParam = params.get('name');
      const arabicParam = params.get('arabic');
      const targetParam = params.get('target');

      // Priority 2: Check Saah Ijabah preset
      const saahPresetStr = localStorage.getItem('asrar_tasbih_preset');

      // Priority 3: Check Zikr Levels Calculator preset
      const levelTargetStr = localStorage.getItem('asrarhub_custom_tasbih_target');
      const levelTitleStr = localStorage.getItem('asrarhub_custom_tasbih_title');

      let initialZikr: Zikr | null = null;
      ...
      } else if (saahPresetStr) {
        try {
          const saahObj = JSON.parse(saahPresetStr);
          if (saahObj && saahObj.text) {
            initialZikr = {
              id: saahObj.id || `saah_${Date.now()}`,
              text: saahObj.text,
              arabic: saahObj.arabic,
              target: saahObj.target || 100,
              category: saahObj.category || 'Sa\'ah Ijabah'
            };
          }
        } catch (e) {}
        localStorage.removeItem('asrar_tasbih_preset');
      } else if (levelTargetStr && levelTitleStr) {
        const targetVal = parseInt(levelTargetStr, 10) || 100;
        initialZikr = {
          id: `calculator_${Date.now()}`,
          text: levelTitleStr,
          target: targetVal,
          category: lang === 'fr' ? 'Calculé' : lang === 'ha' ? 'Lissafa' : 'Calculated'
        };
        localStorage.removeItem('asrarhub_custom_tasbih_target');
        localStorage.removeItem('asrarhub_custom_tasbih_title');
      ...
      if (initialZikr) {
        setActiveZikr(initialZikr);
        setTarget(initialZikr.target);
        setCount(0);

        const exists = [...DEFAULT_ZIKRS, ...customArr].some(z => z.text === initialZikr!.text);
        if (!exists) {
          setAllZikrs([initialZikr, ...DEFAULT_ZIKRS, ...customArr]);
        } else {
          setAllZikrs([...DEFAULT_ZIKRS, ...customArr]);
        }
      } else {
        setAllZikrs([...DEFAULT_ZIKRS, ...customArr]);
      }
      ...
      <div className="flex items-center justify-between mb-6 flex-shrink-0">
        <div className="flex items-center gap-3">
          <Link 
            ...
            className="p-2 -ml-2 rounded-full hover:bg-gray-200 dark:hover:bg-gray-800 text-gray-600 dark:text-gray-300 transition-colors"
          >
            <ArrowLeft size={24} />
          </Link>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-3xl p-5 border border-gray-100 dark:border-gray-700 shadow-sm space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              >
                ...
                <p className="text-[11px] text-gray-500 dark:text-gray-300 leading-snug">
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[11px] text-gray-500 dark:text-gray-300 truncate mt-0.5">
                          {soundDesc}
                        </p>
                      </div>
                    </div>
          ...
          {history.length === 0 ? (
            <div className="bg-white dark:bg-gray-800 rounded-3xl p-8 text-center border border-gray-100 dark:border-gray-700 shadow-sm flex flex-col items-center">
              <div className="w-16 h-16 bg-gray-50 dark:bg-gray-700 rounded-full flex items-center justify-center mb-4">
                <HistoryIcon size={32} className="text-gray-300 dark:text-gray-300" />
              </div>
              <p className="text-gray-500 dark:text-gray-300 font-medium">Aucune session enregistrée.</p>
              ...
              <div className="p-6 sm:p-8 overflow-y-auto space-y-4 max-h-[50vh] scrollbar-thin">
                {activeCircles.length === 0 ? (
                  <div className="text-center py-10">
                     ...
                     <p className="text-sm font-semibold text-gray-600 dark:text-gray-300">{tLocal('noCircles')}</p>
